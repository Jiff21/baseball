...
class CalculationService:
    ...
    def calculate_per_pa_rates(self, hitting_stats: Dict) -> Dict:
        ...
        rates = {
            ...
            'strikeouts_per_pa': hitting_stats.get('strikeouts', 0) / plate_appearances,
            'runs_per_pa': hitting_stats.get('runs', 0) / plate_appearances,
            'walks_per_pa': hitting_stats.get('walks', 0) / plate_appearances,
            'singles_per_pa': hitting_stats.get('singles', 0) / plate_appearances,
            'doubles_per_pa': hitting_stats.get('doubles', 0) / plate_appearances,
            'triples_per_pa': hitting_stats.get('triples', 0) / plate_appearances,
            'home_runs_per_pa': hitting_stats.get('home_runs', 0) / plate_appearances,
            'total_bases_per_pa': hitting_stats.get('total_bases', 0) / plate_appearances,
            'ibb_per_pa': hitting_stats.get('intentional_walks', 0) / plate_appearances,
            'hbp_per_pa': hitting_stats.get('hit_by_pitch', 0) / plate_appearances,
            'rbis_per_pa': hitting_stats.get('rbis', 0) / plate_appearances,
            'stolen_bases_per_pa': hitting_stats.get('stolen_bases', 0) / plate_appearances,
            'caught_stealing_per_pa': hitting_stats.get('caught_stealing', 0) / plate_appearances,
        }
    ...
    def _get_zero_rates(self, hitting_stats: Dict) -> Dict:
        ...
        return {
            ...
            'split_type': hitting_stats.get('split_type', ''),
            'season': hitting_stats.get('season', 2024),
            'strikeouts_per_pa': 0.0,
            'runs_per_pa': 0.0,
            'walks_per_pa': 0.0,
            'singles_per_pa': 0.0,
            'doubles_per_pa': 0.0,
            'triples_per_pa': 0.0,
            'home_runs_per_pa': 0.0,
            'total_bases_per_pa': 0.0,
            'ibb_per_pa': 0.0,
            'hbp_per_pa': 0.0,
            'rbis_per_pa': 0.0,
            'stolen_bases_per_pa': 0.0,
            'caught_stealing_per_pa': 0.0,
            'pa_per_inning': 0.0,
            'wins_per_game': 0.0,
            'losses_per_game': 0.0,
        }
    ...
    def calculate_expected_stats(self, 
        ...
        expected_stats = {
            ...
            'expected_runs': expected_pa * calculated_rates.get('runs_per_pa', 0),
            ...
            'expected_home_runs': expected_pa * calculated_rates.get('home_runs_per_pa', 0),
            'expected_walks': expected_pa * calculated_rates.get('walks_per_pa', 0),
            'expected_strikeouts': expected_pa * calculated_rates.get('strikeouts_per_pa', 0),
            'expected_total_bases': expected_pa * calculated_rates.get('total_bases_per_pa', 0),
            'expected_rbis': expected_pa * calculated_rates.get('rbis_per_pa', 0),
            'expected_stolen_bases': expected_pa * calculated_rates.get('stolen_bases_per_pa', 0),
            'expected_caught_stealing': expected_pa * calculated_rates.get('caught_stealing_per_pa', 0),
            'expected_ibb': expected_pa * calculated_rates.get('ibb_per_pa', 0),
            'expected_hbp': expected_pa * calculated_rates.get('hbp_per_pa', 0),
        }
    ...
    def _calculate_fantasy_points(self, expected_stats: Dict, scoring_settings: Dict) -> float:
        """
        Calculate expected fantasy points based on expected statistics and scoring settings.
        ...
        """
        batting_settings = scoring_settings.get('batting', {})
        
        fantasy_points = 0.0
        
        # Batting statistics
        fantasy_points += expected_stats.get('expected_singles', 0) * batting_settings.get('S', 0)
        fantasy_points += expected_stats.get('expected_doubles', 0) * batting_settings.get('D', 0)
        fantasy_points += expected_stats.get('expected_triples', 0) * batting_settings.get('T', 0)
        fantasy_points += expected_stats.get('expected_home_runs', 0) * batting_settings.get('HR', 0)
        fantasy_points += expected_stats.get('expected_walks', 0) * batting_settings.get('BB', 0)
        fantasy_points += expected_stats.get('expected_ibb', 0) * batting_settings.get('IBB', 0)
        fantasy_points += expected_stats.get('expected_hbp', 0) * batting_settings.get('HBP', 0)
        fantasy_points += expected_stats.get('expected_runs', 0) * batting_settings.get('R', 0)
        fantasy_points += expected_stats.get('expected_rbis', 0) * batting_settings.get('RBI', 0)
        fantasy_points += expected_stats.get('expected_stolen_bases', 0) * batting_settings.get('SB', 0)
        fantasy_points += expected_stats.get('expected_caught_stealing', 0) * batting_settings.get('CS', 0)
        fantasy_points += expected_stats.get('expected_strikeouts', 0) * batting_settings.get('SO', 0)
        fantasy_points += expected_stats.get('expected_total_bases', 0) * batting_settings.get('TB', 0)
