...
def create_app(config_name='development'):
    ...
    def refresh_data():
        """Refresh hitting stats data from MLB APIs."""
        try:
            # Get season from request or use current year
            data = request.get_json() or {}
            season = data.get('season', datetime.now().year)
            
            logger.info(f"Starting data refresh for season {season}")
            ...
            logger.info(f"Successfully refreshed data: {len(stored_teams)} team-split combinations stored")
            return jsonify({
                'success': True,
                'message': f'Successfully refreshed hitting stats for {len(stored_teams)} team-split combinations',
                'stored_teams': len(stored_teams),
                'calculated_teams': len(calculated_teams),
                'season': season
            })
